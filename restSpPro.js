...
const findMin =(...numbers) =>
   numbers.reduce((min, nextV) => min < nextV ? min : nextV);

//mergeObjects
//Write a function called mergeObjects that accepts two objects and 
//returns a new object which contains all the keys and values of 
...
const doubleAndReturnArgs = (arr, num1, num2) =>{
    let dblN1 = num1 * 2;
    let dblN2 = num2 * 2;
    let doubleArr = [...arr, dblN1, dblN2]
    return doubleArr;
};
...
const removeRandom = (item) =>{
    let idxR = Math.floor(Math.random() * (item.length));
    return item.filter(val => {
        return val !== item[idxR];
    })
}
